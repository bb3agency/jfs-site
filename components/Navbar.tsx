      ...
      >
        <div className="flex flex-col h-full p-8">
          <div className="flex justify-between items-center mb-12">
             <span className="text-3xl font-black text-slate-900 tracking-tighter">JFS.</span>
          ...
          <div className="flex flex-col gap-8 flex-1">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`flex items-center gap-6 text-3xl font-bold uppercase tracking-tight transition-colors ${
                  location.pathname === link.path ? 'text-yellow-500' : 'text-slate-900'
                }`}
              >
                {link.icon}
                {link.name}
      ...
      >
        ...
        >
            ...
            <Link to="/" className="relative z-10 flex items-center gap-3 group select-none">
                ...
                <span className="font-black text-xl tracking-tighter text-white drop-shadow-sm">
