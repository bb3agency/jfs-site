...
const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart }) => {
  ...
  return (
    ...
    >
      ...
      {/* Quick Add Button (Bottom Left overlay on image) */}
      <button
        onClick={handleQuickAdd}
        className="absolute bottom-20 left-4 h-10 w-10 bg-slate-900 text-white rounded-xl flex items-center justify-center shadow-lg transition-transform duration-300 scale-0 group-hover:scale-100 hover:bg-slate-800 z-20"
      ...
      <div className="absolute bottom-3 left-3 right-3 bg-white/90 backdrop-blur-xl border border-white/50 rounded-2xl p-3 shadow-lg flex items-end justify-between gap-2 transition-colors group-hover:bg-white">
